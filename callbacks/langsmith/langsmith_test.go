...
import (
	...
	mock_langsmith "github.com/cloudwego/eino-ext/libs/acl/langsmith/mock"
...
func TestLangsmithCallbackDemo(t *testing.T) {
	...
	mockClient := mock_langsmith.NewMockLangsmith(ctrl)
	...
	mockey.PatchConvey("Test Langsmith Callback Demo", t, func() {
		mockey.Mock(langsmith.NewLangsmith).Return(mockClient).Build()
		...
		ctx = SetTrace(ctx,
			WithProjectName("My-Eino-Project"),
...
func TestLangsmithCallback_Stream(t *testing.T) {
	...
	mockey.PatchConvey("Test Langsmith Callback Stream", t, func() {
		...
		mockClient.EXPECT().CreateRun(gomock.Any(), gomock.Any()).DoAndReturn(func(ctx context.Context, run *langsmith.Run) error {
			assert.Equal(t, "My-Streaming-Project", run.ProjectName)
			return nil
		}).Times(1)
		...
		// 设置 trace 信息
		ctx = SetTrace(ctx, WithProjectName("My-Streaming-Project"))
